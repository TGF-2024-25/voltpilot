...
import { VistaEstacionConectores, VistaEstacionInfo, VistaEstacionComentarios, VistaEstacionFotos } from "../views";
...
const EstacionTabView = () => {
  ...
  useEffect(() => {
    const obtenerImagenEstacion = async (name) => {
      try {
        const response = await fetch(`http://127.0.0.1:5001/voltpilot-456010/us-central1/api/getChargerPhoto`, {
          method: "POST",
        ...
        <View style={styles.infoRow}>
          ...
          <Text
            style={[
              ...
              {
                color:
                  selectedCargador.currentOpeningHours?.openNow === true ||
                  selectedCargador.regularOpeningHours?.openNow === true
                    ? "green"
                    : selectedCargador.currentOpeningHours?.openNow === false ||
                        selectedCargador.regularOpeningHours?.openNow === false
                      ? "red"
