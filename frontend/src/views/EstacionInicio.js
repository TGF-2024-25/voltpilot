...
import { useCargador } from "../context/CargadorContext";
...
import MarcadoresInfo from "./MarcadoresInfo"; // Asegúrate de que la ruta sea correcta
import EstacionFiltro from "./EstacionFiltro"; // Asegúrate de que la ruta sea correcta
...
export default function VistaEstacionInicio() {
  ...
  const obtenerCargadores = async (lat, lng, rad) => {
    try {
      const response = await fetch(`http://127.0.0.1:5001/voltpilot-456010/us-central1/api/getChargers`, {
        method: "POST",
    ...
    const cargadoresFiltrados = cargadores.filter((cargador) => {
      ...
      const tieneConectorValido = cargador.evChargeOptions.connectorAggregation.some((conector) => {
        ...
        const cumpleKwhMinimo =
          conector.maxChargeRateKw !== undefined &&
          conector.maxChargeRateKw !== null &&
          conector.maxChargeRateKw >= filtros.minKwh;
      ...
      <GestureHandlerRootView>
        ...
        <BottomSheetModalProvider>
          <BottomSheetModal
            ...
            style={styles.bottomSheetModal} // Agregar estilo para zIndex
          >
